  ...
  <section class="config-layout">
    ...
    <div class="section-stack">
      <ActionPanel eyebrow="Tool Inventory" title="已配置工具">
        <EmptyState v-if="!tools.length" title="暂无工具" description="保存 MCP 或 HTTP 工具后会显示在这里。" compact />
        <div v-else class="inline-list"><div v-for="item in tools" :key="item.id" class="surface-strip"><strong>{{ item.name }}</strong><p class="muted">{{ item.toolType }} · {{ item.endpoint }}</p></div></div>
      </ActionPanel>
      <ActionPanel eyebrow="Skill Inventory" title="已注册技能">
        <EmptyState v-if="!skills.length" title="暂无技能" description="注册本地 Skill 后会显示在这里。" compact />
        <div v-else class="inline-list"><div v-for="item in skills" :key="item.id" class="surface-strip"><strong>{{ item.name }}</strong><p class="muted">{{ item.description }}</p></div></div>
      </ActionPanel>
    </div>
  </section>
</template>
