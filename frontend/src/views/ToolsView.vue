    ...
    <div class="section-stack">
      <ActionPanel eyebrow="Tool Inventory" title="已配置工具">
        ...
        <div v-else class="inline-list">
          <div v-for="item in tools" :key="item.id" class="surface-strip">
            <div class="title-row"><strong>{{ item.name }}</strong><span class="status-badge" :class="{ 'is-success': item.enabled }">{{ item.enabled ? 'ENABLED' : 'DISABLED' }}</span></div>
            <p class="muted">{{ item.toolType }} · {{ item.endpoint }}</p>
          </div>
        </div>
      </ActionPanel>
      <ActionPanel eyebrow="Skill Inventory" title="已注册技能">
        ...
        <div v-else class="inline-list">
          <div v-for="item in skills" :key="item.id" class="surface-strip">
            <div class="title-row"><strong>{{ item.name }}</strong><span class="status-badge" :class="{ 'is-success': item.enabled }">{{ item.enabled ? 'ENABLED' : 'DISABLED' }}</span></div>
